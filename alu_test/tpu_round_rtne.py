...
class TPUFloatOperationTester:
    ...
    def print_test_values_description(self):
        """打印 round.rtne 测试值定义：四舍五入到最近，恰好 .5 时取最近的偶数"""
        if self.dtype == jnp.float32:
            print("\nFP32 测试值定义（round.rtne：ties to even）：")
            print("- nan / inf / -inf / zero / -zero")
            print("- normal: 1.5       （→ 2.0）")
            print("- -normal: -1.5     （→ -2.0）")
            print("- positive_half_odd: 2.5   （→ 2.0，偶数）")
            print("- negative_half_odd: -2.5  （→ -2.0，偶数）")
            print("- positive_half_even: 3.5  （→ 4.0，偶数）")
            print("- negative_half_even: -3.5 （→ -4.0，偶数）")
            ...
            print("- exact_half: 0.5          （→ 0.0，取最近偶数 0）")
            print("- -exact_half: -0.5        （→ 0.0，取最近偶数 0）")
            ...
            print("\nBF16 测试值定义（round.rtne：ties to even）：")
            print("- nan / inf / -inf / zero / -zero")
            print("- normal: 1.5, -normal: -1.5")
            print("- positive_half_odd: 2.5, negative_half_odd: -2.5")
            print("- positive_half_even: 3.5, negative_half_even: -3.5")
    ...
    def create_test_values(self) -> Dict[str, jnp.ndarray]:
        ...
        if self.dtype == jnp.float32:
            ...
            # round.rtne测试的特殊值 (round to nearest, ties to even)
            values['positive_half_odd'] = jnp.array(2.5, dtype=self.dtype)  # -> 2.0 (even)
            values['negative_half_odd'] = jnp.array(-2.5, dtype=self.dtype)  # -> -2.0 (even)
            values['positive_half_even'] = jnp.array(3.5, dtype=self.dtype)  # -> 4.0 (even)
            values['negative_half_even'] = jnp.array(-3.5, dtype=self.dtype)  # -> -4.0 (even)
            ...
            values['exact_half'] = jnp.array(0.5, dtype=self.dtype)  # -> 0.0 (even)
            values['-exact_half'] = jnp.array(-0.5, dtype=self.dtype)  # -> 0.0 (even)
            ...
            # round.rtne测试的特殊值
            values['positive_half_odd'] = jnp.array(2.5, dtype=self.dtype)
            values['negative_half_odd'] = jnp.array(-2.5, dtype=self.dtype)
            values['positive_half_even'] = jnp.array(3.5, dtype=self.dtype)
            values['negative_half_even'] = jnp.array(-3.5, dtype=self.dtype)
    ...
    def test_round_rtne_operations(self) -> List[Tuple[str, str]]:
        """测试round.rtne运算 - round to nearest, ties to even"""
        ...
        test_cases = [
            ...
            'normal',  # 1.5 -> 2.0
            '-normal',  # -1.5 -> -2.0
            'positive_half_odd',  # 2.5 -> 2.0 (to even)
            'negative_half_odd',  # -2.5 -> -2.0 (to even)
            'positive_half_even',  # 3.5 -> 4.0 (to even)
            'negative_half_even',  # -3.5 -> -4.0 (to even)
        ...
        def round_rtne_op(x):
            ...
            # JAX的round函数默认就是round half to even (银行家舍入)
            return jnp.round(x)
    ...
    def print_results(self, results: List[Tuple[str, str]], operation: str = "round.rtne"):
        """打印结果"""
...
def main():
    """在TPU上运行所有测试"""
    print("Starting TPU Float ROUND.RTNE Operation Tests")
    ...
    fp32_results = fp32_tester.test_round_rtne_operations()
    fp32_tester.print_results(fp32_results, "round.rtne")
    ...
    bf16_results = bf16_tester.test_round_rtne_operations()
    bf16_tester.print_results(bf16_results, "round.rtne")
    ...
    fp32_dict = {f"round.rtne({v})": res for v, res in fp32_results}
    bf16_dict = {f"round.rtne({v})": res for v, res in bf16_results}
