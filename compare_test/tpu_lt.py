...
class GPUFloatOperationTester:
    ...
    def test_lt_operations(self) -> List[Tuple[str, str, str]]:
        """测试小于运算"""
        ...
        test_pairs = [
            ...
            # Zero comparisons - +0和-0被认为是相等的，所以<返回False
            ('zero', 'zero'),
            ('zero', '-zero'),
            ('-zero', '-zero'),
            ('zero', 'normal'),
    ...
    def print_results(self, results: List[Tuple[str, str, str]], operation: str = "lt"):
        """打印结果"""
...
def main():
    """在GPU上运行所有测试"""
    print("Starting GPU Float LT Operation Tests")
    ...
    fp32_results = fp32_tester.test_lt_operations()
    fp32_tester.print_results(fp32_results, "lt")
    ...
    bf16_results = bf16_tester.test_lt_operations()
    bf16_tester.print_results(bf16_results, "lt")
    ...
    fp32_dict = {f"{v1} lt {v2}": res for v1, v2, res in fp32_results}
    bf16_dict = {f"{v1} lt {v2}": res for v1, v2, res in bf16_results}
