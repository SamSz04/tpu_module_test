...
class TPUFloatOperationTester:
    ...
    def test_ceil_operations(self) -> List[Tuple[str, str]]:
        """测试ceil运算 - ceil是单操作数运算"""
    ...
    def print_results(self, results: List[Tuple[str, str]], operation: str = "ceil"):
        """打印结果"""
...
def main():
    """在TPU上运行所有测试"""
    print("Starting TPU Float CEIL Operation Tests")
    ...
    fp32_results = fp32_tester.test_ceil_operations()
    fp32_tester.print_results(fp32_results, "ceil")
    ...
    bf16_results = bf16_tester.test_ceil_operations()
    bf16_tester.print_results(bf16_results, "ceil")
    ...
    fp32_dict = {f"ceil({v})": res for v, res in fp32_results}
    bf16_dict = {f"ceil({v})": res for v, res in bf16_results}
