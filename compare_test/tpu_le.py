...
class TPUFloatOperationTester:
    ...
    def test_le_operations(self) -> List[Tuple[str, str, str]]:
        """测试小于等于运算"""
        ...
        # 测试用例
        test_pairs = [
            # NaN comparisons - 任何与NaN的比较都返回False
            ('nan', 'nan'),
            ('nan', 'inf'),
            ('nan', '-inf'),
            ('nan', 'normal'),
            ...
            # Zero comparisons - +0和-0被认为是相等的，所以<=返回True
            ('zero', 'zero'),
            ('zero', '-zero'),
            ('-zero', '-zero'),
            ('zero', 'normal'),
            ...
            # Normal value comparisons - 包含相等的情况
            ('normal', 'normal'),
            ('normal', '-normal'),
            ('-normal', 'normal'),
            ...
            ('normal', 'near_overflow'),
            ('near_overflow', 'normal'),
            ('near_overflow', 'max_float'),
            ('-normal', '-near_overflow'),
    ...
    def print_results(self, results: List[Tuple[str, str, str]], operation: str = "le"):
        """打印结果"""
...
def main():
    """在TPU上运行所有测试"""
    print("Starting TPU Float LE Operation Tests")
    ...
    fp32_results = fp32_tester.test_le_operations()
    fp32_tester.print_results(fp32_results, "le")
    ...
    bf16_results = bf16_tester.test_le_operations()
    bf16_tester.print_results(bf16_results, "le")
    ...
    fp32_dict = {f"{v1} le {v2}": res for v1, v2, res in fp32_results}
    bf16_dict = {f"{v1} le {v2}": res for v1, v2, res in bf16_results}
